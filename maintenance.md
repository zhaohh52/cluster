...
解决办法：重启mu01和计算节点，发现挂载有问题，只有10.10.10.231的IB可用， 其他IB地址不可用（231、233、234及235），进入inspur01节点，执行 ctdb ip,显示如下，执行ctdb status显示，表示存储节点IB掉线，执行systemctl restart ctdb及systemctl restart ganesha，等待一会儿，可以ping通五个IB地址，查看ctdb状态，systemctl status ctdb。再次查看ctdb ip显示如下。   
